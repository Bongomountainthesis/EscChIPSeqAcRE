...
#get DeSeq data for K9ac peak changes

peaks <- read.csv(file = "results/DeSeq_K9ac/peak_compare_middlepeak.csv")
...
k9_rest <- merge(restbind, res_all.od, by.x = "EnsemblID", by.y = "EnsemblID", suffixes = c("_REST","_K9Ac"))
...
for(i in 1:length(k9_rest[,1])){
	k9_rest[i,31] <- k9_rest[i,"Peak_width_K9Ac"] / 2
	k9_rest[i,32] <- (k9_rest[i,"Peak_start_K9Ac"] + k9_rest[i,31]) - 250
	k9_rest[i,33] <- (k9_rest[i,"Peak_end_K9Ac"] - k9_rest[i,31]) + 250
	k9_rest[i,34] <- paste("chr",paste(paste(k9_rest[i,"Chromosome_K9Ac"],k9_rest[i,32],sep = ":"),k9_rest[i,33],sep = "-"), sep ="")
}

k9_rest <- k9_rest[,c(1:30,34)]
colnames(k9_rest)[31] <- "Peak_window_DeSeq"


k9_rest_deseq <- merge(k9_rest, peaks, by.x = "Peak_window_DeSeq", by.y = "Peak")

# save

write.csv(k9_rest_deseq, file = "results/H3K9ac_vs_REST_binding_vs_DeSeqK9ac_middlepeak.csv")

########## plot change in H3K9ac vs REST and try to draw correlation

# remove REST (line 202) from dataset and any other outliers

k9vsRest <- k9_rest_deseq[which(!(k9_rest_deseq[,"Symbol"] == "Rest")),]

plot(k9vsRest[,"FoldEnrichment_REST"], k9vsRest[,"logFC_DeSeq"])

#cor(k9vsRest[,"FoldEnrichment_REST"], k9vsRest[,"logFC_DeSeq"])

fit <- lm(k9vsRest[,"logFC_DeSeq"] ~ k9vsRest[,"FoldEnrichment_REST"])
...
fitrob <- rlm(k9vsRest[,"logFC_DeSeq"] ~ k9vsRest[,"FoldEnrichment_REST"] + 0)
...
postscript(file = "results/DeSeq_K9ac/REST_vs_K9ac_changes.ps", horizontal = FALSE)
smoothScatter(k9vsRest[,"FoldEnrichment_REST"], k9vsRest[,"logFC_DeSeq"], main = "Correlation between REST binding and change in H3K9ac", xlab = "Amount of REST bound", ylab = "Change in H3K9ac")
...
k9_rest_deseq_exp <- merge(k9vsRest, exp.odt, by.x = "EnsemblID", by.y = "EnsemblID")
...
k9_deseq <- merge(res, peaks, by.x = "Peak_window_DeSeq", by.y = "Peak")

# merge with expression data

k9_deseq_exp <- merge(k9_deseq, exp.odt, by.x = "EnsemblID", by.y = "EnsemblID")

## now can plot change in K9ac vs gene expression

plot(k9_deseq_exp[,"logFC"],k9_deseq_exp[,"logFC_DeSeq"])

## looks nicer in hexbin

library(hexbin)

bin <- hexbin(k9_deseq_exp[,"logFC"],k9_deseq_exp[,"logFC_DeSeq"],xbins = 50)

postscript(file = "results/DeSeq_K9ac/change_in_h3k9ac_versus_gene_expression.ps", horizontal = FALSE)
plot(bin, main = "Change in H3K9ac versus Gene Expression", xlab = "Gene Expression (log Fold Change)", ylab = "Change in H3K9ac")
dev.off()


## remove REST and Syt4 and plot change in K9Ac and change in gene expression between WT and REST-KO

fitrob <- rlm(k9_rest_deseq_exp[,"logFC"] ~ k9_rest_deseq_exp[,"logFC_DeSeq"] + 0)

postscript(file = "results/DeSeq_K9ac/exp_vs_h3k9ac_direct_REST_targets.ps", horizontal = FALSE)
plot(k9_rest_deseq_exp[,"logFC"], k9_rest_deseq_exp[,"logFC_DeSeq"], xlab = "Gene Expression (log Fold Change)", ylab = "Change in H3K9ac", main = "H3K9ac versus Gene Expression - direct REST targets", pch =20)
abline(fitrob, lty = 2, lwd = 3, col = "red")
dev.off()
